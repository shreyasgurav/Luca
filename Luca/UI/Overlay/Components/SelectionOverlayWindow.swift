...
final class SelectionOverlayView: NSView {
    ...
    private func makeToolbar() -> NSView {
        ...
        // Add Luca logo
        let logoImageView = NSImageView(frame: CGRect(x: 8, y: 6, width: 24, height: 24))
        logoImageView.image = NSImage(named: "LucaLogoBlack")
        ...
        for (i, b) in [send, copy, cancel].enumerated() {
            ...
            b.frame = CGRect(x: 40 + CGFloat(i) * 72, y: 6, width: 64, height: 24)
            view.addSubview(b)
        }
        view.frame = CGRect(x: 0, y: 0, width: 40 + 3*72, height: 36)
        return view
    }
