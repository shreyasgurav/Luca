...
struct MemoryManagementView: View {
    ...
    private func deleteMemory(_ memory: VectorMemory) async {
        ...
        let success = await memoryManager.deleteVectorMemory(memoryId: memory.id)
        
        // If deletion failed, re-add the memory to the UI
        if !success {
            memories.append(memory)
            // Note: In a production app, you'd want to show an error message to the user
            print("⚠️ Failed to delete memory from backend, restored to UI")
        }
    }
    
    private func clearAllMemories() async {
        // Optimistically clear UI first for instant feedback
        let memoriesToDelete = memories
        memories.removeAll()
        
        // Then delete from backend
        var failedDeletions: [VectorMemory] = []
        for memory in memoriesToDelete {
            let success = await memoryManager.deleteVectorMemory(memoryId: memory.id)
            if !success {
                failedDeletions.append(memory)
            }
        }
        
        // Re-add any memories that failed to delete
        if !failedDeletions.isEmpty {
            memories.append(contentsOf: failedDeletions)
            print("⚠️ Failed to delete \(failedDeletions.count) memories from backend, restored to UI")
        }
    }
}
...
class MemoryManagementWindow {
    ...
    private func createWindow() {
        ...
        window?.title = "Memory Manager - Luca"
